...
parse_result_store(const grok_match_t gm,instanceData *pData)
{
        ...
        while(grok_match_walk_next(&gm,&pname,&pname_len,&pdata,&pdata_len) == 0)
        {
            ...
            if(key!=NULL)
            {
                ...
                while(it)
                {
                    if(key_len == ((result_t *)it->data)->key_len)
                    {
                        int len;
                        len = key_len<((result_t *)it->data)->key_len?key_len:((result_t *)it->data)->key_len;
                        if(strncmp(((result_t *)it->data)->key,key,len))
                        {
                            if(!((result_t *)it->data)->value && result->value)
                            {
                                ((result_t *)it->data)->value = result->value;
                                ((result_t *)it->data)->value_len = result->value_len;
                            }
                            is_append = false;
                            break;
                        }
